...
void SubNetwork::getAllIedAddress(map<string, stIedData> &mapIedData)
{
	 QMap<QString,stConnectedAP*>::Iterator it;
	 for (it = mpConnectedAP_.begin(); it != mpConnectedAP_.end(); ++it)
	 {
		 it.value()->getAllIedAddress(mapIedData);
	 }
}
...
void Communication::getAllIedAddress(map<string, stIedData> &mapIedData)
{
	QMap<QString, stSubNetwork*>::iterator it;
	for (it = mpSubNetwork_.begin(); it != mpSubNetwork_.end(); ++it)
	{
		it.value()->getAllIedAddress(mapIedData);
	}
}
...
void Address::getAllIedAddress(map<string, stIedData> &mapIedData,QString iedname)
{ 
	...
	for (it = mpAddress_.begin(); it != mpAddress_.end(); ++it)
    {
        if (it.key() == "IP")
        {
            if (mapIedData.find(iedname.toStdString()) == mapIedData.end())
            {
                stIedData tmpIedData;
                tmpIedData.IpA_ = it.value().toStdString();
                tmpIedData.PortA_ = 102; 
                mapIedData[iedname.toStdString()] = tmpIedData;
            }
            else
            {
                mapIedData[iedname.toStdString()].IpB_ = it.value().toStdString();
                mapIedData[iedname.toStdString()].PortB_ = 102;
            }
        }
	}
}
...
void ConnectedAP::getAllIedAddress(map<string, stIedData> &mapIedData)
{ 
    if(this->address_ != NULL)
    {
        this->address_->getAllIedAddress(mapIedData, this->iedName_);
    }
}
